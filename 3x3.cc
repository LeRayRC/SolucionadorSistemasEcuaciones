...
TRes3x3 Resolve3x3(TEq3x3 eq1, TEq3x3 eq2,TEq3x3 eq3, bool debug=false){
  ...
  if(res_2x2.code == 0){
    switch(mode){
      case 0: //Resolve x
              if(debug) {printf("Mode %d - resolved y and z on 2x2\n", mode);}
              res.eq.y = res_2x2.x;
              res.eq.z = res_2x2.y;
              if(eq1.x != 0){
                ...
                if(debug){printf("All x are zero\n");}
                res.code = -1;
              }
              break;
      case 1: //Resolve y
              if(debug) {printf("Mode %d - resolved x and z on 2x2\n", mode);}
              res.eq.x = res_2x2.x;
              res.eq.z = res_2x2.y;
              if(eq1.y != 0){
              ...
              break;
      case 2: //Resolve z
              if(debug) {printf("Mode %d - resolved x and y on 2x2\n", mode);}
              res.eq.x = res_2x2.x;
              res.eq.y = res_2x2.y;
              if(eq1.z != 0){
    ...
    res.code = res_2x2.code;
    if(debug){
      printf("Res code %d\n", res.code);
      printf("eq1: %0.2fx %0.2fy %0.2fz = %0.2f\n",eq1.x,eq1.y,eq1.z,eq1.it);
      printf("eq2: %0.2fx %0.2fy %0.2fz = %0.2f\n",eq2.x,eq2.y,eq2.z,eq2.it);
      printf("eq3: %0.2fx %0.2fy %0.2fz = %0.2f\n",eq3.x,eq3.y,eq3.z,eq3.it);
    }
    return {{0.0f,0.0f,0.0f},res.code};
  }
}
